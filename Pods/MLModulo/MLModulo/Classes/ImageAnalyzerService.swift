...
public class ImageAnalyzer: ImageAnalyzerService {
    ...
    public func analyzeImage(image: UIImage, navigationController: UINavigationController, failure: @escaping (_ error: String) -> Void) {
        ...
        guard let model = try? VNCoreMLModel(for: CoelhoTartaruga(configuration: MLModelConfiguration()).model) else {
            failure("can't load CoelhoTartaruga ML model")
            return
        }
        ...
        // Run the Core ML CoelhoTartaruga classifier on global dispatch queue
